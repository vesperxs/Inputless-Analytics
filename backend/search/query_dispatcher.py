...
client = ArangoClient(hosts=tenant_info_dict[0]['arango_host'])
db = client.db(tenant_info_dict[0]['arango_db'],
                username=tenant_info_dict[0]['arango_username'],
                password=tenant_info_dict[0]['arango_password'])
...
def multiscope_query(query_dict):
    print(query_dict)
    try:
...
def decr_query(query_dict):
    try:
        ...
        print(art_num, art_comma,art_opt, art_lett, art_type, num, year)
...
def blacklist(content_to_read):
    ...
    if db.has_collection('BLACKLIST'):
        db.delete_collection('BLACKLIST')
        blist = db.create_collection('BLACKLIST')
    else:
        blist = db.create_collection('BLACKLIST')
